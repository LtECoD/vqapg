...
def _main(args, output_file):
    ...
    for sample in progress:
        ...
        gen_timer.start()
        target_hypos = task.inference_step(
            generator,
            models,
            sample,
        )

        num_generated_tokens = sum(len(h[0]["tokens"]) for h in target_hypos)
        gen_timer.stop(num_generated_tokens)

        for i, sample_id in enumerate(sample["id"].tolist()):
            target_tokens = (
                utils.strip_pad(sample["targets"][i, :], target_dict.pad()).int().cpu()
            )
            ...
            # Process top targets
            if target_hypos is not None:
                for j, hypo in enumerate(target_hypos[i][: args.nbest]):
                    ...
                    if not args.quiet:
                        score = hypo["target_score"] / math.log(2)  # convert to base 2
